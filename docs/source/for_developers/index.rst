...
Clone your fork of the AssetLife repo from your GitHub account to your
...
assetlife directory on your computer with the codebase and every version of every file for
the history of the project:

.. code-block::

  $ git clone https://github.com/YourLogin/assetlife.git  # add --depth 1 if your connection is slow
  $ cd assetlife # go into the directory

.. _upstream:

Next, add the ``upstream`` remote. This saves a reference to the main AssetLife
...
.. code-block:: text
  ...
  origin    https://github.com/YourLogin/assetlife.git (fetch)
  origin    https://github.com/YourLogin/assetlife.git (push)
  upstream  https://github.com/rte-france/assetlife.git (fetch)
  upstream  https://github.com/rte-france/assetlife.git (push)

3. AssetLife local installation
...
the name of your choice. **Activate your virtual environment**. Install AssetLife in
...
For **uv** users, it is possible to install AssetLife in developer or documentation mode with the following commands.
...
latest changes of the main AssetLife repository. To do that, regularly check the evolution
of AssetLife codebase, pull main changes and merge them in your feature branch.
...
for example, if you changed something in ``assetlife/lifetime_model/_conditional_model.py``,
running the following commands will usually be enough:

- ``pytest assetlife/lifetime_model/tests/test_conditional_model.py`` to run the tests specific to the file
- ``pytest assetlife/lifetime_model`` to test the whole ``assetlife.lifetime_model`` module
...
the core team of AssetLife. It is still a work in progress.
...
.. warning::
    ...
    We are aware that overall AssetLife code base *design* can be improved. Especially, we
...
Most of AssetLife methods are Numpy compatible. For typing, we use `optype <https://github.com/jorenham/optype>`__. It offers more functionnalities to type Numpy code and it is compatible with ``scipy-stubs``, the official typing of Scipy.
